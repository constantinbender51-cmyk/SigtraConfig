...
  sendOrder           = async (p) => {
    console.log('DEBUG: Attempting to send order with parameters:', p);
    try {
      const response = await this._request('POST', '/derivatives/api/v3/sendorder', p);
      console.log('DEBUG: Order sent successfully. Response:', response);
      return response;
    } catch (error) {
      console.error('ERROR: Failed to send order.', error);
      throw error;
    }
  };
